...
def build_sku_inventory_dataset(current_product, current_demand, current_inventory, supplier_name):
    sku_names = [
        "Memory Chip", "Copper Laminate", "Microcontroller IC", "Power Module",
        "Connector Set", "Solder Mask", "HDI PCB Core", "Sensor IC",
        "Automotive PCB Assembly", "Testing Fixture"
    ]
    ...
    for i, sku in enumerate(sku_names):
        ...
        lt_mu = random.randint(35, 95)
        lt_sigma = random.randint(5, 28)
        stock = random.randint(800, 9000)

        if sku.lower() in current_product.lower() or i == st.session_state.step % len(sku_names):
            daily_mu = max(40, int(current_demand / 30))
            stock = max(200, int(current_inventory))
            lt_mu = max(30, 60 + supplier_delay)
            lt_sigma = max(5, 8 + manual_supplier_delay)

        base_rows.append({
            "SKU": sku,
            "Component Family": ["Semiconductor", "Laminate", "IC", "Power", "Connector",
                                 "Chemical", "PCB Core", "Sensor", "Assembly", "Testing"][i],
            "Supplier": supplier_name if i % 3 == 0 else random.choice(suppliers),
            "Unit Cost": unit_cost,
...
def build_supplier_performance_dataset():
    supplier_names = [
        ...
        "Local_Backup_Supplier", "Premium_Expedite_Supplier"
    ]
    rows = []
    for s in supplier_names:
        avg_lt = random.randint(35, 95)
        lt_std = random.randint(4, 28)
        ...
        if s == "Local_Backup_Supplier":
            avg_lt = random.randint(20, 45)
            lt_std = random.randint(3, 10)
            cost_index = random.uniform(1.05, 1.35)
            regional_risk = random.uniform(0.05, 0.18)

        if s == "Premium_Expedite_Supplier":
            avg_lt = random.randint(18, 38)
            lt_std = random.randint(2, 8)
            cost_index = random.uniform(1.20, 1.55)
            fill_rate = random.uniform(0.86, 0.99)
            otif = random.uniform(0.84, 0.99)
...
# ---------------- Lead Time Intelligence values used by Reduction Simulator ----------------
# Actual lead time from the Intelligence Layer is used as the "Before Lead Time"
# in the Lead Time Reduction Simulator. This connects both modules logically.
...
total_expected_lead_time = (
    expected_supplier_lead_time +
    expected_fabrication_lead_time +
    expected_pcb_inventory_lead_time +
    expected_logistics_lead_time
)
total_actual_lead_time = (
    actual_supplier_lead_time +
    actual_fabrication_lead_time +
    actual_pcb_inventory_lead_time +
    actual_logistics_lead_time
)
...
# Here, "actual" means current disrupted lead time from the Intelligence Layer.
# "predicted" means lead time after applying the selected recovery strategy.
physical_virtual_gap = abs(total_actual_lead_time - lead_time_after_strategy)
twin_sync_accuracy = max(
    0,
    min(100, 100 - (physical_virtual_gap / max(total_actual_lead_time, 1) * 100))
)
